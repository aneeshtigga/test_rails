...
RSpec.describe "Api::V1::SupportInfos", type: :request do
  describe "GET /api/v1/support_info" do
    context "get support info by office_code" do
      ...
      it "returns with 401 for unauthorized request" do
        token_encoded_get("/api/v1/support_info", params: { office_code: "" }, token: nil)

        expect(response).to have_http_status(401)
      end

      it "returns with 404 when support info is missing for requested office_code" do
        token_encoded_get("/api/v1/support_info", params: { office_code: 12_345 }, token: @token)
      ...
      it "returns support information for valid office_code with data" do
        ...
        token_encoded_get("/api/v1/support_info", params: { office_code: support_info.license_key }, token: @token)
      ...
      context "when license key rule enable credit card on file exists" do
        it "returns an array of support information" do
          ...
          allow_any_instance_of(SupportInfoSerializer).to receive(:heartland_api_key).and_return("abc")
          token_encoded_get("/api/v1/support_info", params: { office_code: support_info2.license_key }, token: @token)
      ...
      context "when marketing referral passed as PPP" do
        it "returns intake_call_in_number as (312) 761-8365" do
          ...
          token_encoded_get("/api/v1/support_info", params: { office_code: support_info2.license_key, marketing_referral: "PPP" }, token: @token)
