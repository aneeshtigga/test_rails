...
RSpec.describe PatientsCustomDataWorker, type: :worker do
  before :all do 
  ...
  describe "PatientsCustomDataWorker" do
    it "should respond to #perform" do
      expect(PatientsCustomDataWorker.new).to respond_to(:perform)
    end
  end
  ...
  describe "perform" do
    context 'Happy path' do
      ...
      it "posts the patients custom data to AMD" do
        Sidekiq::Testing.inline! do
          VCR.use_cassette("amd/patient_custom_data_posting", record: :all) do
            PatientsCustomDataWorker.perform_async(patient.id)
            patient.reload
          end
        end
        expect(patient.amd_patient_id).to eq patient.amd_patient_id

        expect(AuditJob.count).to eq(audit_job_count + 1)
        expect(AuditJob.last.job_name).to eq "PatientsCustomDataWorker"
    ...
    context 'Sad path' do
      ...
      it "ensures the proper AuditJob is created" do
        Sidekiq::Testing.inline! do
          ...
          expect { PatientsCustomDataWorker.perform_async(1) }.to raise_error(StandardError)
          expect(AuditJob.last.job_name).to eq "PatientsCustomDataWorker"
